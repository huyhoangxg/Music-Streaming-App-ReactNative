...
// 5. LẤY DANH SÁCH FOLLOWERS (Những người đang theo dõi user này)
export const getFollowers = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const followers = await prisma.follow.findMany({
      where: { 
        followedId: userId,
        status: 'ACCEPTED' // Chỉ lấy những người đã follow thành công
      },
      include: {
        follower: {
          // Chỉ select những thông tin public cần thiết để hiển thị list
    ...
    // Format lại mảng cho Frontend dễ dùng (bỏ bớt lớp bọc ngoài của bảng Follow)
    const formattedFollowers = followers.map(f => f.follower);

    res.status(200).json(formattedFollowers);
  } catch (error) {
    console.error("Lỗi getFollowers:", error);
    res.status(500).json({ message: 'Lỗi server khi lấy danh sách followers.' });
  }
};

// 6. LẤY DANH SÁCH FOLLOWING (Những người mà user này đang đi theo dõi)
export const getFollowing = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: 'Lỗi server khi lấy danh sách following.' });
  }
};