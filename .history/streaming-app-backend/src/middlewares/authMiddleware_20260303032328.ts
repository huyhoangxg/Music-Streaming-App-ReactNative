...
export const optionalAuthenticateToken = (req: AuthRequest, res: Response, next: NextFunction): void => {
  ...
  if (!token) {
    return next(); // Không có token thì cứ cho đi tiếp (req.user sẽ bị undefined)
  }

  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    if (!err) {
      req.user = decoded as { userId: string }; // Có token hợp lệ thì nhét userId vào
    }
    next(); // Lỗi token hay token xịn thì cũng cho đi tiếp, Controller sẽ tự check
  });
};